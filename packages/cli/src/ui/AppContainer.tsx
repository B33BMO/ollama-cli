  ...
  const handleApiKeySubmit = useCallback(
    async (apiKey: string) => {
      try {
        ...
        // Use the auth type that was selected by the user, not hardcoded USE_GEMINI
        const selectedAuthType = settings.merged.security?.auth?.selectedType;
        if (!selectedAuthType) {
          onAuthError('No authentication type selected');
          return;
        }

        await config.refreshAuth(selectedAuthType as AuthType);
        setAuthState(AuthState.Authenticated);
      } catch (e) {
        onAuthError(
