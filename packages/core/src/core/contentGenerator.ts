...
export async function createContentGeneratorConfig(
  ...
  // Load API key from storage (used by both Gemini and Ollama)
  const storedApiKey = await loadApiKey();

  const geminiApiKey =
    storedApiKey || process.env['GEMINI_API_KEY'] || undefined;
  const googleApiKey = process.env['GOOGLE_API_KEY'] || undefined;
  const ollamaApiKey =
    storedApiKey || process.env['OLLAMA_API_KEY'] || undefined;
  ...
  const generator = await (async () => {
    ...
    if (config.authType === AuthType.USE_OLLAMA) {
      const ollamaClient = new OllamaClient({
        ...
        baseUrl: gcConfig.getOllamaBaseUrl(),
        headers: baseHeaders,
      });
      return new LoggingContentGenerator(
