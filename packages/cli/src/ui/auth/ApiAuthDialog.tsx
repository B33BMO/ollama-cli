...
export function ApiAuthDialog({
  ...
  // Determine if we're using Ollama or Gemini based on settings
  const authType = settings?.merged.security?.auth?.selectedType;
  const isOllama = authType === AuthType.USE_OLLAMA;
  ...
  const buffer = useTextBuffer({
    ...
    inputFilter: (text) =>
      // Allow alphanumeric, underscore, hyphen, and period for API keys
  ...
  return (
    ...
    >
      <Text bold color={theme.text.primary}>
        {isOllama ? 'Enter Ollama API Key' : 'Enter Gemini API Key'}
      </Text>
      <Box marginTop={1} flexDirection="column">
        <Text color={theme.text.primary}>
          {isOllama
            ? 'Please enter your Ollama Cloud API key. It will be securely stored in your system keychain.'
